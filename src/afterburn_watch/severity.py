...
(via ingest.py) when needed, but per guidance from our domain advisor our
primary source is a pre-built NASA RECOVER package (see recover.py), which
already ships a finished dNBR. These functions are still useful to
validate RECOVER's dNBR or to classify severity from a raster we did
compute ourselves.
